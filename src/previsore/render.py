...
def render_terminal(pred: dict, scorers_home=None, scorers_away=None, meta=None) -> str:
    ...
    m = pred.get("markets")
    if m:
        L.append(_row("goals o/u 1.5", ink(f"over {m['over15'] * 100:.0f}%") + faint("  ·  ")
                      + ink(f"under {m['under15'] * 100:.0f}%")))
        L.append(_row("goals o/u 2.5", ink(f"over {m['over25'] * 100:.0f}%") + faint("  ·  ")
                      + ink(f"under {m['under25'] * 100:.0f}%")))
        L.append(_row("goals o/u 3.5", ink(f"over {m['over35'] * 100:.0f}%") + faint("  ·  ")
                      + ink(f"under {m['under35'] * 100:.0f}%")))
